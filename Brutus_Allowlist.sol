...
// File: contracts/Brutus_Chimp_Club.sol
...
// MINT FUNCTIONS -------------------------------------------------
  ...
  modifier mintCompliance(uint8 NUMBER_OF_TOKENS) {
        require(!paused, "The contract is paused!");  
        require(NUMBER_OF_TOKENS > 0, "need to mint at least 1 NFT");  
  ...
  function freeMint(uint8 NUMBER_OF_TOKENS) external payable mintCompliance(NUMBER_OF_TOKENS) {
    ...
    function publicMint(uint8 NUMBER_OF_TOKENS) public payable mintCompliance(NUMBER_OF_TOKENS) {
        if (msg.sender != owner()) {
        if(onlyAllowList == true) {
            require(isAllowListed(msg.sender), "user is not on allow list");
...
// FREE MINT LIST and ALLOW LIST FUNCTIONS -------------------------------------------------

    function setFreeMintList(address[] calldata addresses, uint8 numAllowedToMint) external onlyOwner {
        uint256 LENGTH = addresses.length;
        for (uint256 i = 0; i < LENGTH; i++) {
            FREE_MINT_LIST[addresses[i]] = numAllowedToMint;
        }
    }

    function setAllowList(address[] calldata addresses) external onlyOwner {
        delete ALLOW_LIST;
        ALLOW_LIST = addresses;
        }
    ...
    function isAllowListed(address _receiver) public view returns (bool) {
        uint256 LENGTH = ALLOW_LIST.length;
    for (uint i = 0; i < LENGTH; i++) {
      if (ALLOW_LIST[i] == _receiver) {
          return true;
      }
    }
    return false;
